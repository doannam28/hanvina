...
<path d="M23.5 18L17.5 24L23.5 30" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <li class="page-item {{ $currentPage == 1 ? 'active' : '' }} show-mobile">
            <a class="page-link" href="{{ $paginator->url(1) }}">1</a>
        </li>

        {{-- Hiển thị trang thứ 2 --}}
        @if ($currentPage == 1 || $currentPage == 2)
            <li class="page-item show-mobile {{ $currentPage == 2 ? 'active' : '' }}">
        ...
        @if ($currentPage > 2 && $currentPage < $lastPage)
            @for ($i = $currentPage - 1; $i <= $currentPage + 1; $i++)
                <li class="page-item show-mobile {{ $currentPage == $i ? 'active' : '' }}">
                    <a class="page-link" href="{{ $paginator->url($i) }}">{{ $i }}</a>
                </li>
                <?php $mang[] = $i; ?>
            @endfor
        @endif
        {{-- Dấu "..." nếu có trang phía sau --}}
        @if ($currentPage < $lastPage - 1)
            <li class="page-item show-mobile disabled"><span class="page-link">...</span></li>
        @endif

        {{-- Hiển thị trang cuối --}}
        @if ($lastPage > 1 && !in_array($lastPage, $mang))
            <li class="page-item page-item-last show-mobile {{ $currentPage == $lastPage ? 'active' : '' }}">
