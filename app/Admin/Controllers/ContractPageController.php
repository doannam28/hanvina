...
class ContractPageController extends BaseAdminController
{
    /**
    ...
    {
        return redirect('/admin/contact-page/3/edit');
    }
    ...
    {
        $page = Page::where('type', Page::CONTACT_PAGE)->first();
        ...
        $form->embeds('content_page', "Nội dung", function ($form) {
            $form->image('image', 'image')->attribute(['id' => 'content_page_image']);
            $form->text('title', 'Tiêu đề')->attribute(['id' => 'content_page_title']);
            $form->text('content', 'Nội dung')->attribute(['id' => 'content_page_content']);
        });
